...
static int output_unmasked_data(FILE *out, DASMOutput *dout, dasm_state *D, unsigned a, unsigned z, const char *label)
{
        int started_line=-1;
        ...
        if(!D->mask[a] && (D->mask[a+2] || (a+2 == z)) && !D->mask[a+1] &&
           dout->get_label(a+1)==NULL) {
                ...
                if(addr > CODE_OFFSET && l)
                        fprintf(out, "%-15s DW  %-17s; %04X %02X%02X\n", ihateC, l, a, D->rom[a], D->rom[a+1]);
                else
